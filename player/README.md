...
Not implemented: Back navigation (`allowPrevious`), timer sounds, the `interaction` sub-object on hotspots (undocumented), learner-scoped variables across files (the spec says an offline player treats them as module-scoped), the code sandbox, AI conversation.
...
The engine is a plain state machine. The UI calls `start()`, then `choose()`, `answerText()`, `activateHotspot()`, `tapScene()`, `performStep()`, `continue()` and so on, and reads `view()` to know what to draw. A clock is injected, so timers are tested with a fake one. Everything without a DOM runs unchanged in Node, which is what the tests use.
